...
- The user-authorized Codex runtime is currently YOLO (`approval_policy = "never"`, `sandbox_mode = "danger-full-access"` in the user config); keep the repository hooks fail-closed even in that mode.
...
- Library builds commonly used in this repo:
  - `npm run build:stark-core`
  - `npm run build:stark-ui`
- App validation:
  - `cd showcase && npm run lint && npm run test-fast:ci && npm run build:prod:ghpages`
  - `cd starter && npm run lint && npm run test-fast:ci && npm run build`
