...
No standards finding requires a content change or restack before publication.
...
The review found two documentation/control gaps: the temporary fork archives lacked an explicit removal condition, and the deferred fresh-consumer guide execution was not represented as a publication-blocking decision. D-011 and D-012 now record those decisions, while Beads tasks `stark-4sp.7.1` and `stark-4sp.7.2` block publication.
...
| D-011 | External migration forks     | Keep exact commit-pinned archives for code-style, eslint-config, UI-Router, and ngx-form-errors until equivalent Angular 22-compatible releases are published | Repository-external local paths are not reproducible, while the pinned commits contain compatibility work not yet available from the approved registry versions                                                                                                                                                                            | Clean installs remain reproducible but temporarily resolve four packages from immutable fork commits    | `stark-4sp.7.2` verifies provenance and release availability before publication; replace each archive with an approved published version as soon as its required fixes are released |
| D-012 | Fresh-consumer guide gate    | Keep the fresh Stark 12 consumer execution deferred but make it a hard publication prerequisite                                                               | The guide has not yet been executed end to end from a clean Stark 12-style consumer, so documenting the residual risk is not sufficient for release                                                                                                                                                                                        | No migration branch publication until the documented install, compile, test, and build flow succeeds    | `stark-4sp.7.1` executes the guide and records/fixes every missing or ambiguous step before publication                                                                             |
...
- Compatibility: Keep the same public package versions and document any temporary fork that downstream applications must consume. Remove each commit archive when an approved published version contains the required Angular 22 compatibility fixes; `stark-4sp.7.2` verifies that condition before publication.
...
- [-] a fresh downstream application can follow the migration guide successfully; see D-012 and `stark-4sp.7.1`;
