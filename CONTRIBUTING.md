...
The reusable libraries live in `packages/`, the reference applications live in `starter/` and `showcase/`, and cross-package orchestration lives in `scripts/`. Generated packages and application output are written below `dist/` and must not be edited as source.

## Building from source

If you want to build Stark from source, you need to...

- install Node.js 22.22.3 or newer and npm 10.9.4 or newer, within Angular 22's supported engine range
- clone this git repository: `git clone https://github.com/NationalBankBelgium/stark.git`
- run `npm ci` from the root directory of Stark
...
- execute `npm run update:showcase` to synchronize the rebuilt packages into the showcase
- execute `npm run update:starter` to synchronize the rebuilt packages into the starter

Stark's cross-platform Node build orchestrator:

- builds every Stark package in dependency order using the Angular package builder;
- generates Angular Package Format output and local tarballs;
- synchronizes those tarballs and their lockfile integrity values into the starter and showcase;
- supports focused builds such as `npm run build:stark-core` and tracing through `npm run build:trace`.
...
- run it: `npm start`
- open the URL printed by Angular's development server
- make your changes

Angular's development server rebuilds the application and applies supported HMR updates.

The tracked showcase manifest consumes local Stark tarballs generated by the root build. Rebuild and run `npm run update:showcase` after changing a package.
...
The tracked starter manifest consumes local Stark tarballs generated by the root build. Rebuild and run `npm run update:starter` after changing a package.
...
```
fix($compile): preserve case-insensitive errors

Normalize the received message before comparing it so the assertion reflects
the public behavior instead of a browser-specific serialization detail.
