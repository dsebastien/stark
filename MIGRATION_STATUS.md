...
- The pinned Chromium Playwright/axe runner and legacy-shell smoke journey are integrated; candidate smoke passes with local Stark packages. Beads retains the browser tasks as blocked by prerequisite stack-validation beads.
...
1. Complete the prerequisite `.9.8`/`.1.4` stack validation so the browser foundation beads can close.
2. `stark-4sp.10.2`: validate public UI-Router routing, lazy loading, and session/RBAC transitions.
3. Continue `.9.4` through `.9.8` local tarball workflow work without bypassing dependency guards.
