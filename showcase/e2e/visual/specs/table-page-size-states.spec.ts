import type { Locator } from "@playwright/test";
import { executableScenariosForRunner, type TablePageSizeStateScenario } from "../manifests/scenarios";
import { expect, stabilizeVisualPage, test } from "../fixtures/visual-test";
import { openRouteFromShowcaseShell } from "../support/navigation";

const tableRoute = {
	id: "table",
	menuTrail: ["menu-stark-ui-components", "menu-stark-ui-components-table"],
	path: "/demo-ui/table"
} as const;
type PageSizeCheckpoint = TablePageSizeStateScenario["payload"]["checkpoints"][number];

async function expectPageSizeCheckpoint(
	component: Locator,
	payload: TablePageSizeStateScenario["payload"],
	checkpoint: PageSizeCheckpoint
): Promise<void> {
	const rows = component.locator("tbody tr:not(.expandable-row)");
	await expect(rows).toHaveCount(checkpoint.expectedRowIds.length);
	await expect(rows.locator("td.mat-column-id")).toHaveText([...checkpoint.expectedRowIds]);
	const counter = component.locator(".stark-table-rows-counter");
	await expect(counter).toHaveText(payload.counterText);
	await expect(counter.locator("span").first()).toHaveText(String(payload.totalItems));
	const pagination = component.locator("stark-pagination");
	const select = pagination.locator(".pagination-items-per-page mat-select");
	await expect(select).toHaveText(String(checkpoint.expectedSize));
	await expect(select).toHaveAttribute("aria-expanded", "false");
	const pageInput = pagination.locator("input");
	await expect(pageInput).toHaveValue(String(checkpoint.expectedPage));
	if (payload.viewport.width === 390) {
		await expect(pageInput).toBeHidden();
	} else {
		await expect(pageInput).toBeVisible();
	}
	await expect(pagination.locator(".total-pages")).toHaveText(String(checkpoint.expectedTotalPages));
	for (const [selector, disabled] of [
		[".first-page button", checkpoint.expectedPage === 1],
		[".previous button", checkpoint.expectedPage === 1],
		[".next button", checkpoint.expectedPage === checkpoint.expectedTotalPages],
		[".last-page button", checkpoint.expectedPage === checkpoint.expectedTotalPages]
	] as const) {
		const button = pagination.locator(selector);
		await expect(button).toHaveCount(1);
		if (disabled) {
			await expect(button).toBeDisabled();
		} else {
			await expect(button).toBeEnabled();
		}
	}
	for (const column of ["id", "title", "description"] as const) {
		const header = component.locator(`th.mat-column-${column} .sort-header`);
		const icon = header.locator("mat-icon");
		const sort = checkpoint.expectedSort[column];
		await expect(icon).toHaveAttribute("svgicon", sort.direction === "desc" ? "arrow-down" : "arrow-up");
		if (sort.direction === "none") {
			await expect(icon).toHaveClass(/(?:^|\s)order-tip(?:\s|$)/u);
		} else {
			await expect(icon).not.toHaveClass(/(?:^|\s)order-tip(?:\s|$)/u);
		}
		if (sort.priority === null) {
			await expect(header.locator(".priority")).toHaveCount(0);
		} else {
			await expect(header.locator(".priority")).toHaveText(String(sort.priority));
		}
	}
}

for (const scenario of executableScenariosForRunner("table-page-size-states")) {
	test(`visual-${scenario.id}`, async ({ page }) => {
		await openRouteFromShowcaseShell(page, tableRoute);
		await page.setViewportSize(scenario.payload.viewport);
		await stabilizeVisualPage(page);
		const fixture = page.locator(scenario.payload.fixtureSelector);
		const component = fixture.locator("stark-table");
		const pagination = component.locator("stark-pagination");
		const select = pagination.locator(".pagination-items-per-page mat-select");
		const listbox = page.locator('.cdk-overlay-pane [role="listbox"]');
		await expect(fixture).toHaveCount(1);
		await expect(component).toHaveCount(1);
		await expect(select).toHaveCount(1);
		await fixture.scrollIntoViewIfNeeded();
		await expect(select).toBeVisible();
		// Wait for the asynchronous toolbar icon asset before capturing the whole table.
		await expect(component.locator('.header mat-icon[svgicon="sort"] svg path')).toBeVisible();

		for (const checkpoint of scenario.payload.checkpoints) {
			const { action } = checkpoint;
			switch (action.kind) {
				case "next":
					await pagination.locator(".next button").click();
					break;
				case "sort-id":
					await component.locator("th.mat-column-id .sort-header").click();
					break;
				case "choose-size":
				case "keyboard-five":
					if (action.kind === "keyboard-five") {
						await expect(select).toBeFocused();
						await select.press("Space");
					} else {
						await select.click();
					}
					await expect(listbox).toHaveCount(1);
					await expect(listbox).toBeVisible();
					await expect(listbox.getByRole("option")).toHaveText(scenario.payload.options.map(String));
					if (action.kind === "keyboard-five") {
						await page.keyboard.press("Home");
						await page.keyboard.press("Enter");
						await expect(select).toBeFocused();
					} else {
						await listbox.getByRole("option", { name: String(action.size), exact: true }).click();
					}
					await expect(listbox).toHaveCount(0);
					break;
			}
			await expectPageSizeCheckpoint(component, scenario.payload, checkpoint);
		}

		await expect(listbox).toHaveCount(0);
		const capture = page.locator(scenario.capture.selector);
		await expect(capture).toHaveCount(1);
		await expect(capture).toBeVisible();
		await capture.scrollIntoViewIfNeeded();
		if (scenario.payload.viewport.width === 390) {
			// Center the twelve-row state clear of the Showcase's fixed footer toolbar.
			await capture.evaluate((element) => element.scrollIntoView({ block: "center", inline: "nearest" }));
		}
		await page.mouse.move(0, 0);
		await stabilizeVisualPage(page);
		const screenshotOptions = {
			animations: "disabled",
			caret: "hide",
			mask: scenario.maskSelectors.map((selector) => page.locator(selector)),
			maxDiffPixels: scenario.maxDiffPixels,
			threshold: scenario.threshold
		} as const;
		await expect(capture).toHaveScreenshot(scenario.snapshotName, screenshotOptions);
		if (scenario.payload.selectorSnapshotName !== null) {
			// The legacy mobile header places this control beyond the table host's crop.
			const selectorCapture = pagination.locator(".pagination-items-per-page");
			await expect(selectorCapture).toHaveCount(1);
			await selectorCapture.scrollIntoViewIfNeeded();
			await page.mouse.move(0, 0);
			await stabilizeVisualPage(page);
			await expect(selectorCapture).toBeVisible();
			await expect(selectorCapture).toBeInViewport({ ratio: 1 });
			await expect(select).toHaveAttribute("aria-expanded", "false");
			await expect(listbox).toHaveCount(0);
			await expect(selectorCapture).toHaveScreenshot(scenario.payload.selectorSnapshotName, screenshotOptions);
		}
	});
}
